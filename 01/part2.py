...
def run_on_input(ipt):
    ...
    for line in ipt.split():
        ...
        for spelled_out_digit, digit in spelled_out_digits.items():
            start_idx = 0
            while (digit_index:=line.find(spelled_out_digit, start_idx)) != -1:
                digits_to_add.append((digit_index, digit))
                start_idx = digit_index + len(spelled_out_digit)
        
        offset = 0
        digits_to_add.sort()
        for digit_index, digit in digits_to_add:
            line = line[:(digit_index + offset)] + digit + line[(digit_index + offset):]
            offset += 1

        digits = [ch for ch in line if ch.isdigit()]
        calibration_digits.append(int(digits[0] + digits[-1]))
