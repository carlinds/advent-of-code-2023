...
def run_on_input(ipt):
    ...
    symbols = {(i, j): [] for i in range(len(board)) for j in range(len(board[0])) if board[i][j] in SYMBOLS}
    
    for i, row in enumerate(board):
        for num in re.finditer(r'\d+', row):
            neighbours = {(i_, j) for i_ in (i-1, i, i+1) for j in range(num.start()-1, num.end()+1)}

            for node in neighbours:
                if node in symbols:
                    symbols[node].append(int(num.group()))
