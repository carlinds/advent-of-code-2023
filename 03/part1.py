...
def run_on_input(ipt):
    ...
    for line in ipt.split("\n")[1:-1]:
        is_symbol.append([c in symbols for c in line])
        is_number.append([c.isdigit() for c in line])
        chars.append([c for c in line])

    is_symbol = np.array(is_symbol)
    is_number = np.array(is_number)
    chars = np.array(chars)

    is_adjacent_to_symbol = convolve2d(is_symbol, np.ones((3,3)), mode="same").astype(bool) * is_number
    is_adjacent = convolve2d(is_adjacent_to_symbol, np.ones((1,3)), mode="same").astype(bool) * is_number
    is_adjacent = convolve2d(is_adjacent, np.ones((1,3)), mode="same").astype(bool) * is_number                
    
    numbers = []
    chars[~is_adjacent] = "."
    for row in chars:
        numbers.extend([int(c) for c in "".join(row).split(".") if c.isdigit()])
    print(sum(numbers))
