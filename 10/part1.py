...
def traverse(grid, xs, ys, dx, dy, next_pipe):
    x, y = xs, ys
    n_steps = 1
    while True:
        
        move = decode_pipe(next_pipe, dx, dy)
        ...
        next_pipe = grid[y, x]
        assert next_pipe != ".", "Ran into wall"
...
def run_on_input(ipt):
    ...
    start_pipe = None
    for d, pipes in zip([(0, -1), (1, 0), (0, 1), (-1, 0)],
                        [("|", "F", "7"), ("-", "J", "7"), ("|", "L", "J"), ("-", "F", "L")]):
        if grid[ys+d[1], xs+d[0]] in pipes:
            start_pipe = grid[ys+d[1], xs+d[0]]
            dx, dy = d
            break

    n_steps = traverse(grid, xs + dx, ys + dy, dx, dy, start_pipe)
    print(n_steps/2)
